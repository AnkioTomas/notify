...
新增一个通知渠道时，需要在企业微信后台同步新增一个应用，并完成以下配置：

1. 在企业微信中创建新应用，获取该应用的 `AgentId` 和 `Secret`，填入本系统的渠道配置。
2. 在该应用中开启「接收消息」能力，配置 API 接收消息：
   - **URL**：`https://你的域名/hook`
   - **Token**：使用本系统企业微信页面生成/维护的统一 Token
   - **EncodingAESKey**：使用本系统企业微信页面生成/维护的统一 AES Key
3. 将新应用页面中的 Token / AES Key 更新为与系统一致的值。
4. 配置企业微信「IP 白名单 / 服务器白名单」，确保当前服务所在出口 IP 已加入。

注意：

- 所有渠道对应的企业微信应用，**统一使用同一套** `Token` 和 `EncodingAESKey`。
- 回调地址统一为 `域名/hook`，不要为不同渠道配置不同回调路径。
